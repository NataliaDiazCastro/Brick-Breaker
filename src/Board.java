...
//Class definition
public class Board extends JPanel implements Runnable,ActionListener, Constants {
	//Items on-screen
	private int w = 0,h = 0;
        private Paddle paddle;
	private Ball ball;
	private Brick[][] brick1 = new Brick[9][4];
        private Brick[][] brick2 = new Brick[9][7];
        private Brick[][] brick3 = new Brick[9][10];
	...
	private Thread game;
        ...
        private static JMenu tiempo;
        //private static manejaTime manejador;
	...
	public Board(int width, int height) {
		...
		playerName = JOptionPane.showInputDialog(null, "Por favor Ingrese su nombre:", "Brick Breaker, Version 1.2", JOptionPane.QUESTION_MESSAGE);
		if (playerName == null) {
			System.exit(0);
		}
		...
		withSound = JOptionPane.showOptionDialog(null, "Brick Breaker\n\nControls\n    Barra Espaciadora: Iniciar Juego, Pausar/Reanudar el Juego.\n    Izq/Der teclas: mover barra\nItems\n    Item Verde: Expande la Barra\n     Item Rojo: Acorta la Barra\nPuntajes\n    Bloques: 50 puntos\n    Level-up: 100 puntos\n   Perder Vida: -100 puntos\n\n\n     Quieres escuchar musica de fondo?", "Acerca del Juego", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[1]);
		playMusic(trackList, withSound, level);
                //manejador = new manejaTime();
                time = new Timer(1000, (ActionListener) this);
                time.start();
		game = new Thread(this);
                barra = new JMenuBar();
                tiempo = new JMenu("Tiempo: 0");
                barra.add(tiempo);
	...
	public void makeBricks() {
		...
		for(int i = 0; i < 9; i++) {
			for(int j = 0; j < 4; j++) {
				...
				brick1[i][j] = new Brick((i * BRICK_WIDTH) +20, ((j * BRICK_HEIGHT) ), BRICK_WIDTH - 5, BRICK_HEIGHT - 5, color, numLives, itemType);
			}
		}
            }
            if(level == 2){
		for(int i = 0; i < 9; i++) {
			for(int j = 0; j < 7; j++) {
				...
				int numLives = 3;
				Color color = colors[rand.nextInt(7)][0];
				brick2[i][j] = new Brick((i * BRICK_WIDTH) +20, ((j * BRICK_HEIGHT) ), BRICK_WIDTH - 5, BRICK_HEIGHT - 5, color, numLives, itemType);
			}
		}
            }
            if(level == 3){
		for(int i = 0; i < 9; i++) {
			for(int j = 0; j < 10; j++) {
				...
				int numLives = 3;
				Color color = colors[rand.nextInt(7)][0];
				brick3[i][j] = new Brick((i * BRICK_WIDTH) +20, ((j * BRICK_HEIGHT) ), BRICK_WIDTH - 5, BRICK_HEIGHT - 5, color, numLives, itemType);
			}
		}
            }
	...
	public void run() {
        ...
        public void actionPerformed(ActionEvent ae){
            tiempo1++;
            tiempo.setText("Tiempo: " + tiempo1);
            if (level == 1){
                if(tiempo1 == 100){
                    String[] options = {"yes","No"};
                    int option;
                    option = JOptionPane.showOptionDialog(null, "GAME OVER!!!! \n\n\n\n Desea reiniciar el Juego?","Acerca del Juego",JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0x0]);
                    tiempo1 = 0;
                    ball.reset();
                }
            }
		...
		if (bricksLeft == NO_BRICKS) {
			/*try {
				...
				e.printStackTrace();
                        ...
                        JOptionPane.showInternalMessageDialog(null, "FELICITACIONES PASASTE DE NIVEL");
			ball.reset();
			makeBricks();
			lives++;
			level++;
	...
	public void checkBricks(int x1, int y1) {
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 4; j++) {
				if (brick1[i][j].hitBottom(x1, y1)) {
					ball.setYDir(1);
					if (brick1[i][j].isDestroyed()) {
						bricksLeft--;
						score += 50;
						addItem(brick1[i][j].item);
					}
				}
				if (brick1[i][j].hitLeft(x1, y1)) {
					xSpeed = -xSpeed;
					ball.setXDir(xSpeed);
					if (brick1[i][j].isDestroyed()) {
						bricksLeft--;
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 7; j++) {
				if (brick2[i][j].hitBottom(x1, y1)) {
					ball.setYDir(1);
					if (brick2[i][j].isDestroyed()) {
						bricksLeft--;
						score += 50;
						addItem(brick2[i][j].item);
					}
				}
				if (brick2[i][j].hitLeft(x1, y1)) {
					xSpeed = -xSpeed;
					ball.setXDir(xSpeed);
					if (brick2[i][j].isDestroyed()) {
						bricksLeft--;
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 10; j++) {
				if (brick3[i][j].hitBottom(x1, y1)) {
					ball.setYDir(1);
					if (brick3[i][j].isDestroyed()) {
						bricksLeft--;
						score += 50;
						addItem(brick3[i][j].item);
					}
				}
				if (brick3[i][j].hitLeft(x1, y1)) {
					xSpeed = -xSpeed;
					ball.setXDir(xSpeed);
					if (brick3[i][j].isDestroyed()) {
						bricksLeft--;
	...
	public void playMusic(String[] songs, int yesNo, int level) {
		...
		if (level == 10) {
                    JOptionPane.showInternalMessageDialog(null, "Felicidades has terminado el juego");
			level = 1;
		}
		try {
	...
	public void paintComponent(Graphics g) {
		...
		g.drawString("Puntuaciones: " + score, 10, getHeight() - (2*(getHeight()/10)) + 25);
		...
		if (lives == MIN_LIVES) {
			...
			//	ioe.printStackTrace();
			//}
			g.drawString("Presione Barra Espaciadora para reiniciar", getWidth()/5, getHeight()-20);
		}
	...
	public void writeFakeScores() throws IOException {
		...
		int numLines = 10;
		File f = new File("Puntuaciones.txt");
		BufferedWriter bw = new BufferedWriter(new FileWriter(f.getAbsoluteFile()));
		for (int i = 1; i <= numLines; i++) {
			int score = rand.nextInt(2000);
			if (numLines - i >= 1) {
				bw.write("Nombre: " + "Jugador" + i + ", " + "Puntuacion: " + score + "\n");
			}
			else {
				bw.write("Nombre: " + "Jugador" + i + ", " + "Puntuacion: " + score);
			}
	...
	public String playerInfo() {
		return "Nombre: " + playerName + ", Puntaje: " + score;
	}
	...
	public void sortTable() throws IOException {
		...
		while ((line = br.readLine()) != null) {
			if (line.isEmpty()) {
				...
				String[] scores = line.split("Puntuaciones: ");
	...
	public void printScores(Graphics g) throws IOException {
		...
		int h = 100;
		File fileToRead = new File("Puntuaciones.txt");
...
/*class manejaTime implements ActionListener{
    @Override
    public void actionPerformed(ActionEvent ae){
       int tiempo1 = 0;
       tiempo1++;
